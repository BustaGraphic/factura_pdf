...
const buildHtml = (data = {}) => {
  ...
  const bonoDia =
    Number.isFinite(bonoTotalFront) && bonoTotalFront > 0 && diasN > 0
      ? bonoTotalFront / diasN
      : bonoBaseDia;

  const alquilerDia =
    Number.isFinite(alquilerTotalFront) && alquilerTotalFront > 0 && diasN > 0
    ...
    <div class="py-2.5 px-3.5 grid gap-4 w-full">
      ...
      <div class="bg-[#F8EEE4] h-[90px] flex items-center overflow-hidden justify-between rounded-[20px]">
        <div class="ml-[40px] flex items-center w-auto space-x-[32px] rounded-xl">
          ...
          <div class="text-base text-[#011E37] font-medium">Comparativa Repsol</div>
        </div>
      ...
      <div class="flex flex-col">
        ...
        <div class="text-xs font-medium py-1 -mt-[2px] text-[#6A8298] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-3.5">
          <div class="flex justify-between">
            <div>Concepto</div><div>Importe</div>
          </div>
          <div>Cálculo</div>
          <div>Descripción</div>
        </div>
        ...
        <div class="text-[12px] font-medium py-3 -mt-[2px] text-[#011E37] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-x-3.5 gap-y-2">
          ...
          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between"><div>Período 1</div><div>${euro(importeP1)}</div></div>
              <div class="flex justify-between"><div>Período 2</div><div>${euro(importeP2)}</div></div>
            </div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between w-full"><div>${potenciaKwText} kW</div><div>×</div><div>${diasFactura} días</div><div>×</div><div>${precioKwDiaTextP1}</div></div>
              <div class="flex justify-between w-full"><div>${potenciaKwText} kW</div><div>×</div><div>${diasFactura} días</div><div>×</div><div>${precioKwDiaTextP2}</div></div>
            </div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex justify-between text-[10px]">Es el importe que pagas por estar conectado a la red a tu distribuidora y por la cuota de comercialización a Repsol.</div>
          </div>
        </div>
        ...
        <div class="text-[12px] font-medium py-3 -mt-[2px] text-[#011E37] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-x-3.5 gap-y-2">
          ...
          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between"><div>Consumo (P1)</div><div>${euro(impP1)}</div></div>
              <div class="flex justify-between"><div>Consumo (P2)</div><div>${euro(impP2)}</div></div>
              <div class="flex justify-between"><div>Consumo (P3)</div><div>${euro(impP3)}</div></div>
            </div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex space-x-1 w-full justify-end"><div>${kWh1Text}</div><div>×</div><div>${priceP1Text}</div></div>
            <div class="flex space-x-1 w-full justify-end"><div>${kWh2Text}</div><div>×</div><div>${priceP2Text}</div></div>
            <div class="flex space-x-1 w-full justify-end"><div>${kWh3Text}</div><div>×</div><div>${priceP3Text}</div></div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex justify-between text-[10px]">Es el importe variable que pagas por la electricidad que has utilizado. Se calcula en función de los kWh consumidos y el precio de cada kWh.</div>
          </div>
        </div>
        ...
        <div class="text-[12px] font-medium py-3 -mt-[2px] text-[#011E37] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-x-3.5 gap-y-2">
          ...
          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between"><div>Financiación del Bono Social</div><div>${euro(totalBono)}</div></div>
              <div class="flex justify-between"><div>Alquiler del contador</div><div>${euro(totalAlquiler)}</div></div>
            </div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex space-x-1 w-full justify-end"><div>${diasFactura} días</div><div>×</div><div>${formatNumberComma(bonoDia, 6)} €/día</div></div>
            <div class="flex space-x-1 w-full justify-end"><div>${diasFactura} días</div><div>×</div><div>${formatNumberComma(alquilerDia, 5)} €/día</div></div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex justify-between text-[10px]">Aquí se muestra la financiación del bono social y el alquiler del contador si no es de tu propiedad.</div>
          </div>
        </div>
        ...
        <div class="text-[12px] font-medium py-3 -mt-[2px] text-[#011E37] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-x-3.5 gap-y-2">
          ...
          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between"><div>Impuesto Eléctrico</div><div>${impElecText}</div></div>
              <div class="flex justify-between"><div>IVA (21%)</div><div>${ivaText}</div></div>
            </div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex space-x-1 w-full justify-end"><div>${baseImpElecText}</div><div>×</div><div>${tipoImpElecText}</div></div>
            <div class="flex space-x-1 w-full justify-end"><div>${formatNumberComma(baseIVA, 2)} €</div><div>×</div><div>${tipoIVAText}</div></div>
          </div>

          <div class="flex flex-col space-y-1.5">
            <div class="flex justify-between text-[10px]">Incluimos en tu factura los impuestos regulados, como el Impuesto Eléctrico sobre el consumo y el IVA sobre el total.</div>
          </div>
        </div>
        ...
        <div class="text-[12px] font-medium -mt-[8px] py-3 rounded-[20px] text-[#011E37] px-4 border-[2px] border-[#DBE6F0] grid grid-cols-[1fr_1fr_1fr] gap-x-3.5 gap-y-2">
          ...
          <div class="flex flex-col space-y-1.5">
            <div class="flex flex-col space-y-1">
              <div class="flex justify-between"><div>Gasolina</div><div>15c€/L</div></div>
              <div class="flex justify-between"><div>Tarjeta Regalo</div><div>80€</div></div>
              <div class="flex justify-between"><div>Recarga electrica</div><div>75%</div></div>
            </div>
          </div>
        ...
        <div class="text-[12px] font-medium text-[#011E37] grid grid-cols-7 gap-x-3.5 gap-y-2">
          ...
          <div class="border-[2px] border-[#DBE6F0] w-full rounded-[20px] col-span-4 flex py-3 px-4">
            <div>
              <div class="text-[16px] font-semibold">${name}</div>
